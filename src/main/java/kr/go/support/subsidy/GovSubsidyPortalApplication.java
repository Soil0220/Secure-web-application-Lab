...
public class GovSubsidyPortalApplication {
    ...
    /*시나리오
        ...
        취약설정 : 세션검증 제거, URL에 UserId를 변경해가며 Account 페이지 접근 가능

        [2. Initial Access / Information Disclosure]
        파일 업로드 기능을 이용하여 Path Traversal을 통해 application.yml 혹은 DEV_NOTES 파일에 접근하여 업로드 경로, 관리자API를 획득한다.
        ...
        이를 통해 관리자 세션을 탈취하여 관리자 권한을 확보한다.
        → 문의사항 Stored XSS → 관리자 페이지 방문 → 관리자 브라우저에서 JS 실행 → 세션 탈취 / 관리자 권한 API 요청 → 관리자 권한 획득
        취약설정 : 세션에 대한 HttpOnly 설정 제거, 리액트에서  dangerouslySetInnerHTML 설정을 통한 Stored XSS 동작
        ...
        취약설정 : 정부 지원금 사이트의 특성상 자연스럽게 악용가능

        [5. Second-Order SQL Injection]
        악성 SQL Injection Payload를 로그 데이터에 삽입하여 저장한 후, 관리자가 로그 모니터링 기능을 이용하는 과정에서 저장된 데이터가 동적 SQL 구문에 재사용되도록 유도한다.
